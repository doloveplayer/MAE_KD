...
class ViT(nn.Module):
    def __init__(
        ...
        self.img_h, self.img_w = to_pair(image_size)
        self.patch_h, self.patch_w = to_pair(patch_size)
        assert not self.img_h % self.patch_h and not self.img_w % self.patch_w, \
            f'Image dimensions ({self.img_h},{self.img_w}) must be divisible by the patch size ({self.patch_h},{self.patch_w}).'
        num_patches = (self.img_h // self.patch_h) * (self.img_w // self.patch_w)
